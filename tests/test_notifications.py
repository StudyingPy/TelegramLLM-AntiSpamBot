...
from telegram_llm_antispam.notifications import notify_admins
...
def test_notify_admins_sends_one_combined_record_with_admin_ban_button():
    bot = FakeBot()
    ...
    result = ActionResult(action_log_id=3, vote_session_id=5, deleted=True, banned=False)

    asyncio.run(notify_admins(bot, _settings_with_admins(), features, decision, result))
...
def test_notify_admins_skips_allowed_messages():
    bot = FakeBot()
    message = SimpleNamespace(
        message_id=7,
        chat=SimpleNamespace(id=-100123),
    ...
    asyncio.run(notify_admins(bot, _settings_with_admins(), features, decision, ActionResult()))
